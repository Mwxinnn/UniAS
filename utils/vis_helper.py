...
def visualize_compound(fileinfos, preds, masks, cfg_save_dir, cfg_vis, cfg_reader):
    vis_dir = os.path.join(cfg_save_dir, cfg_vis.SAVE_DIR)
...
def visualize_single(fileinfos, preds, cfg_save_dir, cfg_vis, cfg_reader):
    vis_dir = os.path.join(cfg_save_dir, cfg_vis.SAVE_DIR)
    max_score = cfg_vis.get("max_score", None)
    min_score = cfg_vis.get("min_score", None)
    max_score = preds.max() if not max_score else max_score
